...
// GDKX NOTE: search for "GDKX CHANGE" to see what's different between this and upstream!
...
void* GCToOSInterface::VirtualReserve(size_t size, size_t alignment, uint32_t flags, uint16_t node)
{
    ...
    return ::VirtualAlloc(nullptr, size, memFlags, PAGE_READWRITE); // GDKX CHANGE -- Removed NUMA check
}
...
void* GCToOSInterface::VirtualReserveAndCommitLargePages(size_t size, uint16_t node)
{
    ...
    SIZE_T largePageMinimum = 4096; // GDKX CHANGE -- Seems like pages can be 4K, 64K, or 2MB. Not sure which to use, so 4K it is for now.
    size = (size + (largePageMinimum - 1)) & ~(largePageMinimum - 1);
    return ::VirtualAlloc(nullptr, size, MEM_RESERVE | MEM_COMMIT | MEM_LARGE_PAGES, PAGE_READWRITE); // GDKX CHANGE -- Removed NUMA check
}
...
{
    return ::VirtualAlloc(address, size, MEM_COMMIT, PAGE_READWRITE) != nullptr; // GDKXCHANGE -- Removed NUMA check
}
...
{
    return ::VirtualAlloc(address, size, MEM_RESET, PAGE_READWRITE) != nullptr; // GDKX CHANGE -- Removed VirtualUnlock call
}
...
{
    // GDKX CHANGE -- Not sure what to do here.
    //if (is_restricted)
    //    *is_restricted = false;
